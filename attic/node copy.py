...
def query_node(ip_address: str, subnet_id: int = None) -> dict:
    query: str = (
        "SELECT DISTINCT IpNodeId, SubnetID, Uri FROM IPAM.IPNode WHERE IPAddress = @ip_address"
    )
...
def xget_uri_from_id(node_id: int) -> str:
...
def get_uri_from_id(node_id: int) -> str:
    ...
    result: list = _query_builder(table, fields, params)
...
def get_nodes_in_subnet(subnet_id: int = None) -> list[dict]:
    query: str = (
        "SELECT DISTINCT IpNodeId, SubnetId, IPAddress, IPMapped, Alias, MAC, DnsBackward, DhcpClientName, Comments, ResponseTime, SkipScan, Status, AllocPolicy, Uri FROM IPAM.IPNode WHERE SubnetId = @subnet_id"
    )
