...
class IPAddress:
    ...
    def prepare_updates(self):
        ...
        if self.Status == IpNodeStatus.Available:
            self.IPMapped = self.Alias = self.MAC = self.DnsBackward = (
                self.DhcpClientName
            ) = self.Comments = None
    ...
    def as_dict(self):
        return {
            ...
            for keyname in self.__annotations__
            if keyname in vars(self)
        }
    ...
    def calculate_ordinal(self):
        def to_int(ip_address):
            return sum(
                int(octet) * 256**counter
                for counter, octet in enumerate(reversed(ip_address.split(".")))
            )

        if not getattr(self, "SubnetAddress", None):
            self.SubnetAddress, self.SubnetMask, self.SubnetCIDR = (
                self.get_subnet_address()
            )
