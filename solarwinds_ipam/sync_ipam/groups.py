...
# Group C/R/U/D
# 
def create(friendly_name:str='', parent_id:int=0, subnet_type:SubnetType=SubnetType.Group, **properties:dict)->str:
...
def query_group(subnet_address:str=None, subnet_cidr:int=None, subnet_type:SubnetType=SubnetType.Subnet)->dict:
    query:str = 'SELECT DISTINCT ParentID, SubnetID, Uri, CIDR  FROM IPAM.Subnet WHERE Address = @subnet_address'
...
def get_group_comments(friendly_name:str=None)->str:
    query:str = 'SELECT DISTINCT Comments FROM IPAM.Subnet WHERE FriendlyName = @friendly_name'
